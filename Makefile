...
COFFEE_FILES = CustomDataTypeLoc.coffee \
...
build: clean buildinfojson ## clean, compile, copy files to build folder
			...
			cat src/tmp/CustomDataTypeLoc.js >> build/$(PLUGIN_NAME)/webfrontend/customDataTypeLoc.js
			cat src/tmp/CustomDataTypeLocFacet.js >> build/$(PLUGIN_NAME)/webfrontend/customDataTypeLoc.js
			cat src/tmp/LocUtil.js >> build/$(PLUGIN_NAME)/webfrontend/customDataTypeLoc.js

			cp src/updater/LocUpdater.js build/$(PLUGIN_NAME)/updater/LocUpdater.js # build updater
			cat src/tmp/LocUtil.js >> build/$(PLUGIN_NAME)/updater/LocUpdater.js
			cp package.json build/$(PLUGIN_NAME)/package.json
			cp -r node_modules build/$(PLUGIN_NAME)/
			rm -rf src/tmp # clean tmp

			cp l10n/customDataTypeLoc.csv build/$(PLUGIN_NAME)/l10n/customDataTypeLoc.csv # copy l10n			
